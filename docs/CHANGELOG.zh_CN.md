...
- 优化桌面补给站 0.2.1：收紧标题区、版本下移页脚、突出饭盒食物、统一原生扁平按钮，合并开始/暂停操作。锁定配置仍清晰可读，区分运行状态与胸牌连接，送达时间保留快照含义。喂养、配对、存档和 USB 安全保护不变，补充展示状态回归测试。异步退出改在后台清理完成时确认，不再依赖可能暂停的界面计时器。

- 新增原生中文 Apple Silicon 桌面补给站，内嵌 Python，提供菜单栏启停、可取消 Kaboo 导出、认证送达状态、安全 USB 安装和明确主动开启的登录自启。桌面流程不接官方月结，未对账归档显示本地成长记录，兼容存档不删除。增加隔离服务测试、构建工具及运行依赖许可证。ad-hoc 签名不等于 Developer ID 签名或公证；干净电脑、原厂安装仍需验收。

- 新增中文 macOS 内测安装向导：明确备份／刷写确认、严格分区检查、私有双读备份、仅刷应用及保护区读回。单独提供启动／配对、Kaboo 预检、每台设备独立密钥档案、向导单实例和白名单 ZIP 打包。不内置私有导出器、官方月结连接器或自启服务；Python 安装与干净机器验收仍是明确的内测边界。

- 新增可信本机个人数据连接器驱动的月度归档对账，只补进化、不退化。冻结原日目标、陪伴天数、分支与多宠共享分母，不发本月份食物、不虚构陪伴天数；高水位存档成功才确认，家族页区分待对账与已完成。v3/v4 CRC 记录显式迁到 v5，保留原槽，旧归档仍作为本地记录。覆盖重试、缺失覆盖日期、共享上限、档案淘汰、保存失败、协议边界和真实尺寸界面。

- 新增主动开启、限时的附近伙伴打招呼，通过临时公开 BLE 扫描响应和双方实体按键确认显示两位伙伴与组合彩蛋；页面退出、超时、身份变化或电脑连接时取消。启用 observer，不增加连接槽或凭据；未经认证的相遇数据与饭盒、亲密度、家族存档隔离，覆盖双端模拟、重放、确认、取消及真实尺寸 UI 测试。

- 新增亲密度 30 解锁、主动选择的亚古兽暗黑路线，丧尸暴龙兽与黑暗战斗暴龙兽素材，确认/取消、无损切回原主线、独立形态解锁和准确的月度分支归档。校验通过的 v3 存档在原 CRC 双槽迁到 v4，升级时只改变版本；拒绝未知版本，保留亲密度、配对和原有成长。

- 新增三轮瞄准训练、各主线像素出手反馈、成绩页和可选的每日亲密奖励。所有伙伴共享每日三次奖励，亲密度按主线长期保留，解锁首页招呼和庆祝，不改变饭盒及进化。使用独立 CRC 存档，防止重复和过期成绩，支持保存失败重试及离线练习；验证计时、每日上限、存储故障、页面导航、布局和中断退出。
