...
菜单打开「数码伙伴」。上下键切换首页、今日饭盒、进化条件、路线、家族。单击 OK 吃一份已获得的食物；空饭盒时摸摸或唤醒宠物，不增加成长。进食 1.2 秒，进化演出 1.8 秒。安静 30 秒后休息，不惩罚。家族页单击 OK 翻阅，长按 OK 返回硬件菜单。双击不再产生食物。
...
首次正用量获得食物，此后须严格超过锁定日目标的每个 20% 边界才获得下一份；数据向下修正不扣已经获得的食物。这是定期同步的本地快照，不是实时计数或 Bits 官方总量。重启后尚未同步，或十分钟没有同步，显示「上次的饭盒」并保留来源日期。新食物送到时，首页提示四秒，不打断进食或进化、不自动切页；相同快照不会重复提醒。不改 v2 存档、同步协议或食物额度。

### 三条进化路线

路线页显示当前倾向；单击 OK 轮流预览最终形态，不投食、不改存档，也不是手动选路线。达到 RANGER（12 份已吃食物、6 个用量活跃日）时锁定本月路线，跨月保留在图鉴。三条路线共用成长门槛，没有哪条更高级。

只统计领养以来已经结束的日子，按获得的食物而非点击时机投票：1～2 份为轻量日，3 份为混合日，4～5 份为高能日。轻量日严格多于另外两类分别的天数时走 EXPLORER；高能日严格多于另外两类时走 WILD；混合日最多或并列时走 ARMOR。无食物日不计票。当天额度还在变化，因此不参与；没有完整用量日时显示 CORE / 等待。锁定后不随后续用量或数据修正改变。

这是本地食物强度路线，不代表工作质量、任务深度或工具多样性，也不是 Bits 认证路线。装甲型有蓝色护甲与盾，野性型为橙色身体与红色鬃毛，探索型为绿色身体与红围巾；仍使用七阶段原创程序像素角色。

v2 存档结构不变，以原来未使用的当前月字段 `family.route` 保存路线。之前版本的成年宠物在下一次同步或吃饭时补锁路线，不丢阶段和食物；未达 RANGER 的月度记录仍为 CORE。
...
尚缺 Bits 结算及基于认证特征的路线、独立 Flux 适配、可选物种、相遇、音效和正式精灵素材。真机断电、电池续航和连续三天人工体验属于独立验收，主机模拟不能代替。无线常驻后与 Wi-Fi 扫描、96 KB 录音等硬件演示并发的完整资源验收未完成；日常使用 AI Pet / Pet Link 页面。
