...
function sprite(line,stage,pose) {
...
for(let line=0;line<3;line++) for(let stage=0;stage<7;stage++) for(let pose=0;pose<3;pose++) {
  const pixels=sprite(line,stage,pose), bytes=[];
  ...
  const id=`line${line}_${names[line][stage]}_${pose}`;
...
if(process.argv.includes('--check')) {
  ...
  console.log('Digimon sprites: PASS (3 lines, 7 forms, 3 poses, deterministic RGB565A8 assets)');
} else {writeFileSync(target,source);console.log('Generated 3 lines x 7 forms x 3 poses, 169344 bytes of RGB565A8 pixel data');}
