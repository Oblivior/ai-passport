...
static void play_tone(void) {
    set_status("播放 1kHz 测试音...");
    if (bsp_audio_set_format(SAMPLE_RATE, 16, 1) != ESP_OK) { set_status("音频格式设置失败"); return; }
...
void demo_audio_enter(void) {
    s_scr = ui_pixel_screen_create("声音测试");
    ...
    lv_label_set_text(s_status, "确定：1kHz 测试音\n上键：录音并回放");
